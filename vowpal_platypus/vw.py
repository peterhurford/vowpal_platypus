...
class VW:
    ...
    def vw_base_command(self, base):
        ...
        if self.params.get('quadratic')           is not None: l.append('-q '.join([s for s in self.params['quadratic']]) if is_list(self.params['quadratic']) else '-q ' + str(self.params['quadratic']))
        if self.params.get('cubic')               is not None: l.append('--cubic '.join([s for s in self.params['cubic']]) if is_list(self.params['cubic']) else '--cubic ' + str(self.params['cubic']))
